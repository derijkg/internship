...
def sanitize_name(name):
    """Sanitizes column and model names for safe output folder/file creation."""
    return str(name).replace(":", "_").replace("/", "_").replace("\\", "_").replace(" ", "_")


# Base Static Run Configurations
'''
RUN_CONFIGS = {
    "clin_euro": {
        "model": "euro",
...
def register_abstracts_qwen_configs(llm_cols=None):
    """
    Dynamically registers abstracts_qwen_{llm_col} configurations into RUN_CONFIGS.
    """
    if llm_cols is None:
        llm_cols = TARGET_LLM_COLS

    for col in llm_cols:
        clean_col = sanitize_name(col)
        run_key = f"abstracts_qwen_{clean_col}"
...
# Register dynamic abstract Qwen configs immediately
register_abstracts_qwen_configs(TARGET_LLM_COLS)
...
def load_abstracts_dataset(
    ...
    for idx, row in df_parquet.iterrows():
        ...
        # LLM text from specified columns
    ...
    df = pd.DataFrame(final_records)
    print(f"Extracted {len(df)} random sentences ({sum(df['is_llm']==0)} Human, {sum(df['is_llm']==1)} LLM) from Parquet [Min length: {min_words} words]")
    return df
