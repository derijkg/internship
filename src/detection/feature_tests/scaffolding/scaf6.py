...
def compute_scaffold_log_likelihoods(
    ...
    # 1. Tokenize sentence S independently (guarantees t_1..t_M is fixed across all scaffolds)
    sentence_ids = tokenizer.encode(sentence, add_special_tokens=False)
    M = len(sentence_ids)
    if M == 0:
        raise ValueError("Candidate sentence produced 0 tokens.")

    # 2. Tokenize scaffolds (retaining BOS special tokens)
    scaffold_ids_list = [
    ...
    # 3. Combine scaffolds and target sentence into left-padded batch inputs
    ...
    # Position IDs for explicit RoPE alignment under left padding
    ...
    # 5. Vectorized Log-Likelihood Gathering (VRAM Optimized)
    target_ids = input_ids[:, max_len - M :]  # Shape: (K, M)

    # Slice logits ONLY for target sentence prediction positions BEFORE log_softmax
    # Position max_len - M - 1 predicts t_1; position max_len - 2 predicts t_M
    pred_logits = outputs.logits[:, max_len - M - 1 : max_len - 1, :].float()  # Shape: (K, M, Vocab)
    pred_log_probs = F.log_softmax(pred_logits, dim=-1)

    # Gather log-probabilities for target token IDs
    token_log_probs = pred_log_probs.gather(
        dim=-1, index=target_ids.unsqueeze(-1)
    ).squeeze(-1)  # Shape: (K, M)

    # Sum log probabilities over sequence length M
...
def compute_vocabulary_trajectory_geometry(
    ...
    across vocabulary space.

    Args:
        sentence: Candidate sentence text S.
        model: Observer model.
        tokenizer: Tokenizer.
        top_n: Number of top tokens N (used if alignment="confidence_spectrum").
        use_softmax: If True, uses normalized softmax probabilities; else raw logits.
        alignment: 
            - "vocab_space": Measures directional shifts over the full vocabulary space V.
            - "confidence_spectrum": Measures shifts in the top-N probability decay curve.
    ...
    Returns:
        ...
        trajectory_matrix: Matrix V of shape (M, dim) representing generation path.
    """
    if device is None:
        device = next(model.parameters()).device
    ...
    # Logits at positions 0..M-1 predict target tokens t_1..t_M
    ...
    if alignment == "vocab_space":
        # Full V-dimensional probability vectors aligned by token index
        v_matrix = probs_or_logits
    else:
        # Top-N probability profile sorted by rank
...
class ContextualFrictionDetector:
    ...
    def compute_trajectory_geometry(
        ...
        Computes path curvature angles theta and angular variance Var(theta) in R^V.
        """
