...
class Step3TrajectoryResult:
    ...
    trajectory_matrix: np.ndarray            # Trajectory matrix V of shape (M, N)


@dataclass
class DetectionResult:
    """Dataclass holding complete multi-step detection metrics and decision score D(S)."""
    sentence: str
    num_tokens: int
...
def compute_scaffold_log_likelihoods(
    ...
    Args:
        sentence: Candidate sentence text S.
        model: Pre-trained causal observer LLM (M).
        tokenizer: Pre-trained tokenizer associated with model M.
        scaffolds: List of K synthetic prefix prompt strings (P_1 ... P_K).
        device: Torch compute device. If None, defaults to model's device.

    Returns:
        scaffold_log_likelihoods: Tensor of shape (K,) containing conditional 
                                  log-likelihoods [ell_1(S), ..., ell_K(S)].
        token_log_probs: Tensor of shape (K, M) containing token-level log probabilities.
    """
    if device is None:
        device = next(model.parameters()).device

    # 1. Tokenize sentence S independently (ensuring token sequence t_1..t_M is fixed across all scaffolds)
    sentence_ids = tokenizer.encode(sentence, add_special_tokens=False)
    M = len(sentence_ids)
    if M == 0:
        raise ValueError("Candidate sentence produced 0 tokens.")

    # 2. Tokenize scaffolds (retaining BOS special tokens for prompt initialization)
    scaffold_ids_list = [
    ...
    # 3. Combine scaffolds and target sentence, construct left-padded batch inputs
    ...
    # Construct explicit position_ids to guarantee proper alignment under left-padding
    position_ids = attention_mask.long().cumsum(-1) - 1
    position_ids.masked_fill_(attention_mask == 0, 0)

    # 4. Parallel Forward Pass
    outputs = model(
        input_ids=input_ids,
    ...
    # Cast to float32 for numerical stability in log-softmax operations
    logits = outputs.logits.float()
    log_probs = F.log_softmax(logits, dim=-1)

    # 5. Vectorized log-likelihood gathering
    # Target tokens t_1..t_M reside at index [max_len - M : max_len]
    target_ids = input_ids[:, max_len - M :]  # Shape: (K, M)

    # Logits predicting position j are located at index j - 1
    pred_log_probs = log_probs[:, max_len - M - 1 : max_len - 1, :]  # Shape: (K, M, Vocab)

    # Gather log-probabilities for actual target token IDs
    token_log_probs = pred_log_probs.gather(
        dim=-1, index=target_ids.unsqueeze(-1)
    ).squeeze(-1)  # Shape: (K, M)

    # Sum log probabilities over sequence length M to get ell_k(S)
...
def compute_unconditioned_log_likelihood(
    ...
    for candidate sentence S without any scaffold prefix.

    Returns:
        uncond_log_likelihood: Scalar tensor sum of unconditioned log probabilities.
        uncond_token_log_probs: Tensor of shape (1, M) with token-level log probabilities.
    """
    if device is None:
        device = next(model.parameters()).device
    ...
    # Logits at positions 0..M-1 predict target tokens t_1..t_M at positions 1..M
...
def compute_vocabulary_trajectory_geometry(
    ...
    Extracts top-N logit/probability vectors at each token step i and computes 
    path curvature (differential angles theta_i) across vocabulary space.

    Args:
        sentence: Candidate text S.
        model: Observer model.
        tokenizer: Model tokenizer.
        top_n: Number of top logits/probabilities N to retrieve per step.
        use_softmax: If True, uses normalized softmax probabilities; else raw logits.
        device: Torch compute device.

    Returns:
        angles: Array of shape (M-1,) containing transition angles theta_i in radians.
        theta_var: Variance of transition angles Var(theta).
        theta_mean: Mean of transition angles Mean(theta).
        trajectory_matrix: Matrix V of shape (M, top_n) containing logit/prob vectors.
    """
    if device is None:
        device = next(model.parameters()).device
    ...
    # Logits at positions 0..M-1 predict tokens t_1..t_M
    pred_logits = outputs.logits[:, :-1, :].squeeze(0).float()  # Shape: (M, Vocab)

    if use_softmax:
        probs_or_logits = F.softmax(pred_logits, dim=-1)  # Shape: (M, Vocab)
    else:
        probs_or_logits = pred_logits

    # Retrieve top-N logits/probs at each step i: shape (M, top_n)
    ...
    if M <= 1:
        ...
        return angles, 0.0, 0.0, v_matrix.cpu().numpy()

    # L2-normalize vectors along dimension N
    v_norm = F.normalize(v_matrix, p=2, dim=-1)  # Shape: (M, N)

    # Differential cosine similarity between successive steps v_i and v_{i+1}
    v_i = v_norm[:-1, :]     # Shape: (M-1, N)
    v_next = v_norm[1:, :]   # Shape: (M-1, N)

    cos_sim = (v_i * v_next).sum(dim=-1)  # Shape: (M-1,)
    
    # Clamp cosine similarity for numerical stability in arccos
    ...
    return angles, theta_var, theta_mean, v_matrix.cpu().numpy()
...
class ContextualFrictionDetector:
    ...
    def compute_contextual_friction(
        ...
        Computes length-normalized scaffold profiles, unconditioned baseline subtraction,
        global Contextual Friction Index (CFI), and per-token friction profiles.
        """
        # 1. Retrieve scaffold profile (K, M) and unconditioned baseline (1, M)
        scaffold_ll, token_log_probs = self.get_scaffold_profile(sentence)
        uncond_ll, uncond_token_log_probs = self.get_unconditioned_baseline(sentence)
        ...
        # 3. Differential Resonance Calculation: \Delta R(S) = R(S) - R_uncond(S)
        differential_resonance = resonance_vector - uncond_ll_norm  # Shape: (K,)

        # 4. Global Contextual Friction Index (CFI)
        # Normalized residuals: (R_k - mean(R)) / sigma_P
        R_mean = resonance_vector.mean()
        residuals = (resonance_vector - R_mean) / self.sigma_p
        cfi_global = torch.var(residuals, unbiased=False).item()

        # 5. Per-Token Contextual Friction Profile
        # token_log_probs shape: (K, M)
        # Rescale per-token probabilities if length normalization requested
        T_matrix = token_log_probs  # Shape: (K, M)
        T_mean = T_matrix.mean(dim=0, keepdim=True)  # Shape: (1, M)
        
        # Per-token normalized residual across K scaffolds: (K, M)
        token_residuals = (T_matrix - T_mean) / self.sigma_p.unsqueeze(1)
        per_token_cfi = torch.var(token_residuals, dim=0, unbiased=False).cpu().numpy()  # Shape: (M,)
    ...
    def compute_trajectory_geometry(
        ...
        """
        Step 3: Top-K Vocabulary Trajectory Geometry (Micro-Topology)
        Computes path curvature angles theta and angular variance Var(theta).
        """
    ...
    def analyze_sentence(
        ...
        step3_res = self.compute_trajectory_geometry(sentence, top_n=top_n)
