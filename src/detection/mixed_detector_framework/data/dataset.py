...
# Flexible imports to handle varied project module layouts
try:
    from features.stylometrics import StylometricFeatureEngine
    from features.dense_encoder import DenseTransformerEncoder
except ImportError:
    try:
        from mixdet import StylometricFeatureEngine, DenseTransformerEncoder
    except ImportError:
        pass


# =============================================================================
# 1. SYNTHETIC DOCUMENT DATACLASS & GENERATION ENGINE
# =============================================================================
@dataclass
...
def pad_collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Includes seq_lengths as a 1D CPU tensor to prevent CPU-GPU sync stalls in mixdet.py.
    """
...
# =============================================================================
# 3. UNIFIED BUILDER FUNCTION FOR MIXDET.PY INTEGRATION
# =============================================================================
def build_dataset(config: Any) -> MixedAuthorshipDataset:
    """
    Loads raw parquet data, synthesizes mixed-authorship scenarios, 
    computes multi-scale fused features (or loads cached vectors), and 
    returns a MixedAuthorshipDataset ready for mixdet.py training or tuning.
    """
    print("=========================================================")
    print(" LOADING & PREPARING MIXED-AUTHORSHIP DATASET ")
    print("=========================================================")
    
    data_path = config.data.data_path
    if not os.path.exists(data_path):
        raise FileNotFoundError(f"[ERROR] Parquet data file not found at: {data_path}")

    print(f"Loading parquet raw data from '{data_path}'...")
    df = pd.read_parquet(data_path)

    if config.data.sample_size is not None and config.data.sample_size < len(df):
        print(f"-> Subsampling dataset to {config.data.sample_size} raw documents...")
        df = df.sample(n=config.data.sample_size, random_state=config.data.random_seed).reset_index(drop=True)

    # 1. Synthesize mixed-authorship scenarios
    data_engine = MixedAuthorshipDataEngine(
        random_state=config.data.random_seed,
        min_sentences=config.data.min_sentences
    )
    synthetic_docs = data_engine.process_dataframe(df)

    # 2. Instantiate Feature Extraction Engines
    style_engine = StylometricFeatureEngine(
        include_w3=config.model.include_w3,
        include_w5=config.model.include_w5
    )
