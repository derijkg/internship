...
def objective(trial: optuna.Trial, cfg, dataset) -> float:
    ...
    # =========================================================================
    # 1. OPTUNA HYPERPARAMETER SUGGESTIONS (UPDATED RANGES & NEW PARAMS)
    # =========================================================================
    
    lr = trial.suggest_float("learning_rate", 1e-4, 3e-3, log=True)
    weight_decay = trial.suggest_float("weight_decay", 1e-6, 1e-2, log=True)

    aux_boundary_weight = trial.suggest_float("aux_boundary_weight", 0.2, 3.0)
    ...
    crf_lr_mult = trial.suggest_float("crf_lr_mult", 2.0, 10.0, step=1.0) # Search range: 2x to 10x multiplier

    emission_temp = trial.suggest_float('emission_temp', 0.5, 2.0, step=0.1)

    use_attention = trial.suggest_categorical('use_attention', [True, False])
    
    # =========================================================================
    # 2. FAST SINGLE FOLD SPLIT
    # =========================================================================
    ...
    # =========================================================================
    # 3. BUILD MODEL & OPTIMIZER WITH NEW HYPERPARAMETERS
    # =========================================================================
    # [UPDATED] Pass all new parameters into your neural CRF tagger
    ...
    optimizer = torch.optim.AdamW([
        ...
        {"params": crf_params, "lr": lr * crf_lr_mult, "weight_decay": 0.0}  # Higher LR, NO weight decay!
    ])
    ...
    for epoch in range(max_search_epochs):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # Move GPU tensors back to CPU for numpy operations
        ...
        # PRUNING CHECK: Kills bad trials early
        trial.report(val_auc, step=epoch)
        if trial.should_prune():
            raise optuna.exceptions.TrialPruned()

        # EARLY STOPPING CHECK (Exits plateaued trials early)
        if val_auc > best_val_auc:
            ...
            if patience_counter >= patience:
                print(f"    --> Early stopping trial at epoch {epoch + 1} (Best AUC: {best_val_auc:.4f})")
                break
...
def run_tuning():
    ...
    study = optuna.create_study(
        ...
        pruner=pruner,
        storage=db_file,
        load_if_exists=True  # Loads existing study if database file exists
    )
